...
const PANEL = {
  BASIS: { title: "HTF Bias", subtitle: "Daily & 4-Hour Timeframe", accent: "cyan" },
  ENTRY: { title: "LTF Entry Model", subtitle: "Session Based", accent: "green" },
};
...
export function RulesList({ category }: RulesListProps) {
  ...
  const rules    = state.rules.filter((r) => r.category === category);
  ...
  const accentBg    = category === "BASIS" ? "rgba(106,236,225,0.08)" : "rgba(0,255,127,0.08)";
  ...
  const handleAdd = () => {
    if (input.trim()) {
      dispatch({ type: "ADD_RULE", payload: { label: input.trim(), category } });
      setInput("");
      setAdding(false);
    }
  };
  ...
  const onDrop      = (toIndex: number) => {
    if (dragIndex.current !== null && dragIndex.current !== toIndex) {
      dispatch({ type: "REORDER_RULES", payload: { category, fromIndex: dragIndex.current, toIndex } });
    }
  ...
  return (
    <div
      className="shimmer-card relative overflow-hidden rounded-xl p-5 h-full"
    ...
    >
      ...
      <div className="flex items-start justify-between mb-1">
        <div className="flex items-center gap-2">
          <ChevronRight
            size={17} strokeWidth={2.5}
            className="anim-float"
            style={{ color: accentColor }}
          />
          <h3 className="font-mono font-bold text-white text-base tracking-wide">
      ...
      <div className="space-y-2">
        {rules.map((rule: Rule, index: number) => (
          <div
            ...
            onDrop={() => onDrop(index)}
            onDragEnd={onDragEnd}
            className="group relative anim-fade-up"
            style={{ animationDelay: DELAYS[index] ?? "500ms", animationFillMode: "both" }}
          >
            {/* Drop indicator */}
            {dragOver === index && (
              <div className="absolute -top-1 left-0 right-0 h-0.5 rounded-full" style={{ backgroundColor: accentColor }} />
            )}

            <div
              className="flex items-center gap-3 px-3 py-3 rounded-lg border-l-4 transition-all duration-200"
              style={rule.checked
            ...
            >
              ...
              {/* Check toggle with pop animation */}
              <button
              ...
              >
                {rule.checked ? (
                  <CheckCircle2 size={22} strokeWidth={0} fill={biasColor} />
                ) : (
                  <Circle size={22} strokeWidth={1.5} color="#333" />
                )}
              </button>
              ...
              <button
                onClick={() => handleToggle(rule.id)}
                className={cn(
                  "flex-1 text-left font-sans text-sm transition-all duration-200",
                  rule.checked ? "text-white font-semibold" : "text-[#444] font-normal"
                )}
              >
                {rule.label}
              </button>
              ...
              {/* Delete */}
              <button
                onClick={() => dispatch({ type: "REMOVE_RULE", payload: rule.id })}
                className="opacity-0 group-hover:opacity-100 shrink-0 text-[#333] hover:text-[#FF3B3B] transition-all duration-150"
                title="Delete rule"
              >
                <Trash2 size={13} />
              </button>
        ...
        {adding ? (
          <div className="flex gap-1.5 anim-fade-up">
            ...
            <button onClick={handleAdd}
              className="px-2 py-1.5 rounded font-mono text-xs font-bold text-black"
              style={{ background: accentColor }}>+</button>
            <button onClick={() => { setAdding(false); setInput(""); }}
              className="px-2 py-1.5 text-[#555] rounded text-xs"
              style={{ border: "1px solid #2A2A2A" }}>✕</button>
          </div>
        ) : (
          <button
